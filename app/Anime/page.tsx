...
export default function Movie(){
    ...
    useEffect(() => {
        if (content) {
            ...
            if (mediaBlocks) {
                ...
                const filteredAnime = allMedia.filter((item) => item.contentType === 'anime');
    ...
    return(
        ...
        <div className="gap-[20px] movies ">
            {anime.length === 0 ? (<p className="noneText">Аниме пока нет</p>) : (anime.map((movie, index) => (
                <div key={index} className="bg-[#424549] p-[20px] text-[white] rounded-[15px] cardMovie">
                    ...
                    {movie.rate <= 10 && movie.rate > 6 ? (<p className="bg-[#008000] p-[5px] rounded-[10px] mb-[15px] mt-[10px]">Оценка: {movie.rate}/10</p>) : (movie.rate <= 6 && movie.rate > 3) ? (<p className="bg-[gold] p-[7px] rounded-[10px] mb-[15px] mt-[10px]">Оценка: {movie.rate}/10</p>) : (<p className="bg-[red] p-[7px] rounded-[10px] mb-[15px] mt-[10px]">Оценка: {movie.rate}/10</p>)}
